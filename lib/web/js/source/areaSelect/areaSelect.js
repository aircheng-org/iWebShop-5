...
function areaSelect(topName)
{
	...
	this.child   = [];
	this.defaultText = {};//各个name选择区域默认显示文字
	...
	this.createAreaSelect = function(name,parent_id,select_id)
	{
		...
		var defaultText = _this.defaultText[name] ? _this.defaultText[name] : '请选择';

		//artTemplate模板 {name:组件名字,area_id:选中的地区ID,data:地区的对象}
		areaTemplate = '<option value="">'+defaultText+'</option>'+'<%for(var index in data){%>'+'<%var item = data[index]%>'+'<option value="<%=item.area_id%>" <%if(item.area_id == select_id){%>selected="selected"<%}%>><%=item.area_name%></option>'+'<%}%>';
	...
	this.areaChangeCallback = function(_self)
	{
		...
		{
			var defaultText = _this.defaultText[childArray[index]] ? _this.defaultText[childArray[index]] : '请选择';
			$('select[name="'+childArray[index]+'"]').html('<option value="">'+defaultText+'</option>');
		}
